...
> 上一节说 harness 是模型的缰绳。
> **再深一层：harness 还能反过来——用 agent 去训练这个模型自己。**
>
> 这是当下最前沿的"自进化模型"思想——
> **模型自己造数据、自己评分、自己迭代、自己变强。**
...
1. 拿一堆真实 GitHub issue。
2. 在沙盒里读代码、改代码、跑测试。
3. 测试通过 → reward = 1；不通过 → reward = 0。
4. 把这些 trajectory 喂给 RL → 模型学会"在真实仓库里 debug"。
...
**关键**：reward 不需要人类来标，**测试套件就是 verifier**。
人类被踢出了执行回路——只剩下**"设计这个回路"**的工程师。
...
设计一个自循环训练系统，要回答的问题**没有一个 AI 能替你答**：
...
> 这些问题的答案 **决定了整个系统能不能自我进化**——
> 答错一个，模型在循环里**越训越蠢**（mode collapse / reward hacking / verifier 漂移）。
>
> 高中生连这些问题都问不出来。
...
> Karpathy 说"理解不能外包"。
> 在自循环训练这里，**最不能外包的"理解"就是：**
>
> > **"我设计的这个循环，到底会让模型变强，还是变蠢？"**
>
> 这个问题没有任何 AI 能替你回答。
> 这就是高级工程师在 AI 时代真正的位置——
> **不是写代码的人，不是跑 agent 的人，是设计"AI 自我进化回路"的人。**
